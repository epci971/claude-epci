...
> **Durée estimée**: {total_hours}h ({total_days}j)
...
|---|-----|-------|------|------|-------|------|-------------|--------|--------|
| 1 | US-001 | {title} | S01 | Script | M | P1 | - | 1.5h | ⏳ |
| 2 | US-002 | {title} | S01 | Logic | S | P1 | US-001 | 1h | ⏳ |
| 3 | US-003 | {title} | S01 | Logic | S | P1 | US-001 | 1h | ⏳ |
| ... | ... | ... | ... | ... | ... | ... | ... | ... | ... |
...
### S01 — {Spec Title} ({story_count} stories, {spec_hours}h)

| # | ID | Tâche | Estim. | Dépendances | Status |
|---|-----|-------|--------|-------------|--------|
| 1 | US-001 | {title} | 1.5h | - | ⏳ |
| 2 | US-002 | {title} | 1h | US-001 | ⏳ |
| 3 | US-003 | {title} | 1h | US-001 | ⏳ |
| 4 | US-004 | {title} | 1h | US-003 | ⏳ |

### S02 — {Spec Title} ({story_count} stories, {spec_hours}h)
...
| Échouées | {failed} |
| Restantes | {remaining} |
| Parallélisables | {parallelizable} |
...
_Generated by /decompose — Project: {slug}_
````
...
**Schema version**: 2.0 — Enhanced structure for granular tracking
...
- **RM1**: `passes = true` → `status = "completed"` automatically
...
_Template for prd.json v2 — See `ralph-converter` skill for full inference logic_
