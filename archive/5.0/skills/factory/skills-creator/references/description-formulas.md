...
| Weak | Strong |
|------|--------|
| `Helps with documents` | `Extract text and tables from PDF files, fill forms, merge documents. Use when working with PDFs. Not for image-only PDFs.` |
| `For data analysis` | `Analyze Excel spreadsheets, create pivot tables, generate charts. Use when working with .xlsx files. Not for CSV or DB queries.` |
| `Code helper` | `Review Python code for security vulnerabilities (OWASP). Use when auditing Python files. Not for general refactoring.` |
| `SQL stuff` | `Query data warehouse for revenue, ARR, and customer metrics. Use when analyzing business data. Not for raw event logs.` |
...
print("OK" if len(description) <= 1024 else "TOO LONG")
```
